...
<head>
  ...
  <title>GP RAZ - Pregled garaža</title>
  ...
  <div class="wrapper">
    ...
    <div id="content">
      ...
      <div class="containter">
        <div class="row">
          <div class="col-12">
            <h2><i class="fas fa-warehouse"></i> Pregled garaža po objektima</h2>
            ...
            <?php if ($selektovanId > 0): ?>
              ...
              <div class="mb-4">
                ...
                <?php
                // Učitaj garaže za objekat sa JOIN za kupca
                $garazeCrud = new CRUD($_SESSION['godina']);
                $garazeCrud->table = "garaze";
                $garaze = $garazeCrud->select(['*'], [], "SELECT g.*, k.ime as kupac_ime, k.prezime as kupac_prezime
                  FROM garaze g 
                  LEFT JOIN kupci k ON g.kupac_id = k.id
                  WHERE g.objekat_id = {$selektovanId} 
                  ORDER BY g.id ASC");
                ?>
                  ...
                  <table class="table table-striped table-hover">
                    ...
                    <tbody>
                      <?php if (count($garaze) === 0): ?>
                        <tr>
                          <td colspan="6" class="text-center text-muted">Nema unetih garaža za ovaj objekat.</td>
                        </tr>
                      <?php else: ?>
                        <?php foreach ($garaze as $gar): ?>
                          <tr>
                            <td><?= isset($gar['naziv']) ? htmlspecialchars($gar['naziv']) : ('Garaža #' . (int)$gar['id']) ?></td>
                            <td><strong><?= number_format((float)$gar['cena_sa_pdv'], 2, ',', '.') ?></strong></td>
                            <td><?= number_format((float)$gar['pdv'], 2, ',', '.') ?></td>
                            <td>
                              <?php if ((int)$gar['prodat'] === 1): ?>
                            ...
                            <td>
                              <?php if ((int)$gar['prodat'] === 1 && !empty($gar['kupac_ime'])): ?>
                                <?= htmlspecialchars(trim($gar['kupac_ime'] . ' ' . ($gar['kupac_prezime'] ?? ''))) ?>
                              <?php else: ?>
                                -
                              <?php endif; ?>
                            </td>
                            <td>
                              <?php if ((int)$gar['prodat'] === 1 && !empty($gar['datum_prodaje'])): ?>
                                <?= formatirajDatum($gar['datum_prodaje']) ?>
                              <?php else: ?>
                                -
                              <?php endif; ?>
                            </td>
